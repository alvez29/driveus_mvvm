...
class PayoutsListAdapter(
...
) : ListAdapter<Pair<String, QueryDocumentSnapshot>, PayoutsListAdapter.PayoutViewHolder>(diffCallback) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PayoutsListAdapter.PayoutViewHolder {
        val rideView = LayoutInflater.from(parent.context).inflate(R.layout.row_payout, parent, false)

        return PayoutViewHolder(rideView)
    }

    override fun onBindViewHolder(holder: PayoutsListAdapter.PayoutViewHolder, position: Int) {
